...
parser.add_argument('--scan_path', required=True, help='path to scannet scene (e.g., data/ScanNet/v2')
parser.add_argument('--filelist', required=True, help='path to scannet scene txt file (e.g., datagen/fileLists/scannetv2_val.txt')
parser.add_argument('--output_dir', required=True, help='output file')
parser.add_argument('--label_map_file', required=True, help='path to scannetv2-labels.combined.tsv')
parser.add_argument('--label_map_file2', required=True, help='path to nyu40labels_scannet.csv')
...
def export(mesh_file, agg_file, seg_file, label_map_file, label_map_file2, type, output_file):
    label_map = utils.read_label_mapping(opt.label_map_file, label_from='raw_category', label_to='nyu40id')
    label_map2 = utils.read_label_mapping(opt.label_map_file2, label_from='nyu40id', label_to='mappedIdConsecutive', delimiter=',')
    mesh_vertices = utils.read_mesh_vertices(mesh_file)
    object_id_to_segs, label_to_segs = read_aggregation(agg_file)
    seg_to_verts, num_verts = read_segmentation(seg_file)
    label_ids = np.zeros(shape=(num_verts), dtype=np.uint32)     # 0: unannotated
    for label, segs in label_to_segs.items():
        label_id = label_map2[label_map[label]]
        for seg in segs:
            verts = seg_to_verts[seg]
            label_ids[verts] = label_id

    if type == 'label':
        utils.export_ids(output_file, label_ids)
    elif type == 'instance':
        ...
        utils.export_ids(output_file, label_ids * 1000 + instance_ids)
    else:
        raise


def main():
    filelist = open(opt.filelist).readlines()
    for scan_name in os.listdir(opt.scan_path):
        if scan_name + '\n' not in filelist:
            continue
        print(scan_name)
        #scan_name = os.path.split(opt.scan_path)[-1]
        mesh_file = os.path.join(opt.scan_path, scan_name, scan_name + '_vh_clean_2.ply')
        agg_file = os.path.join(opt.scan_path, scan_name, scan_name + '.aggregation.json')
        seg_file = os.path.join(opt.scan_path, scan_name, scan_name + '_vh_clean_2.0.010000.segs.json')
        export(mesh_file, agg_file, seg_file, opt.label_map_file, opt.label_map_file2, opt.type, os.path.join(opt.output_dir, scan_name + '.txt'))
