...
def assign_instances_for_scan(pred_file, gt_file, pred_path):
    ...
    # get gt instances
    gt_instances = utils.get_instances(gt_ids, VALID_CLASS_IDS, CLASS_LABELS, ID_TO_LABEL)
    # associate
    ...
    for pred_mask_file in pred_info:
        ...
        if len(pred_mask) != len(gt_ids):
            utils.print_error('wrong number of lines in ' + pred_mask_file + '(%d) vs #mesh vertices (%d), please double check and/or re-download the mesh' % (len(pred_mask), len(gt_ids)))
        # convert to binary
...
def main():
    ...
    gt_files = []
    if len(pred_files) == 0:
        utils.print_error('No result files found.', user_fault=True)
    for i in range(len(pred_files)):
        gt_file = os.path.join(opt.gt_path, pred_files[i])
        if not os.path.isfile(gt_file):
            utils.print_error('Result file {} does not match any gt file'.format(pred_files[i]), user_fault=True)
