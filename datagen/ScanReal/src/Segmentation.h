...
public:
	...
	////verticesPerSegment: seg-id -> list of vert ids //TODO THIS IS THE SAME AS getSegmentIdToVertIdMap
	//void computeVerticesPerSegment(std::unordered_map< unsigned int, std::vector<unsigned int> >& verticesPerSegment) const {
	//	verticesPerSegment.clear();
	//	for (unsigned int i = 0; i < m_segmentIds.size(); i++) { //i:vertex id, m_segmentIds[i]:segment id
	//		auto it = verticesPerSegment.find(m_segmentIds[i]);
	//		if (it != verticesPerSegment.end()) it->second.push_back(i);
	//		else verticesPerSegment[m_segmentIds[i]] = std::vector<unsigned int>(1, i);
	//	}
	//}
