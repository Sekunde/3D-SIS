...
template<typename T>
inline void fromJSON(const rapidjson::GenericValue<T>& d, vec3f& out) {
	
	for (unsigned int i = 0; i < 3; i++)
		out[i] = (float)d[i].GetDouble();
}

template<typename T>
inline void fromJSON(const rapidjson::GenericValue<T>& d, bbox3f& out) {
	vec3f bboxmin, bboxmax;
	fromJSON(d["min"], bboxmin);
	fromJSON(d["max"], bboxmax);
	out = bbox3f(bboxmin, bboxmax);
}

template<typename T>
inline void fromJSON(const rapidjson::GenericValue<T>& d, mat4f& out) {
	for (unsigned int c = 0; c < 4; c++) {
		for (unsigned int r = 0; r < 4; r++)
			out(r,c) = (float)d[c*4+r].GetDouble();
	}
}

template<typename T>
inline void fromJSON(const rapidjson::GenericValue<T>& d, std::unordered_map<std::string, std::string>& out) 
{
	for (rapidjson::Value::ConstMemberIterator iter = d.MemberBegin(); iter != d.MemberEnd(); ++iter) {
		out[iter->name.GetString()] = iter->value.GetString();
	}
}
