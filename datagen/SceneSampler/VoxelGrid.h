...
#pragma omp parallel for
	...
	std::unordered_set<vec3i> countProjections(const mat4f& intrinsic, const mat4f& cameraToWorld, const DepthImage32& depthImage) const
	{
		std::unordered_set<vec3i> validProjSet;
			...
			for (int j = voxelBounds.getMinY(); j <= voxelBounds.getMaxY(); j++) {
				for (int i = voxelBounds.getMinX(); i <= voxelBounds.getMaxX(); i++) {
					...
					if (pi.x >= 0 && pi.y >= 0 && pi.x < (int)depthImage.getWidth() && pi.y < (int)depthImage.getHeight()) {
						...
						if (d >= m_depthMin && d <= m_depthMax && std::fabs(d - p.z) <= m_voxelSize) {
							validProjSet.insert(vec3i(i, j, k));
						}
					}
				}
			}
	...
	std::pair<std::unordered_map<unsigned int, std::unordered_set<vec3i>>, unsigned int> countProjectionsInBoxes(const mat4f& intrinsic, const mat4f& cameraToWorld, const DepthImage32& depthImage, const std::vector<BBInfo>& bboxes, const std::vector<float>& bboxInChunk) const
	{
		if (bboxes.empty()) throw MLIB_EXCEPTION("[countProjectionsInBoxes] empty bboxes!");
		const mat4f worldToCamera = cameraToWorld.getInverse();
		BoundingBox3<int> voxelBounds = computeFrustumBounds(intrinsic, cameraToWorld, depthImage.getWidth(), depthImage.getHeight());

		std::unordered_set<vec3i> hitSet;
		std::unordered_map<unsigned int, std::unordered_set<vec3i>> boxesHit; // box id -> # voxels hit for box id
		//#pragma omp parallel for
		for (int k = voxelBounds.getMinZ(); k <= voxelBounds.getMaxZ(); k++) {
			for (int j = voxelBounds.getMinY(); j <= voxelBounds.getMaxY(); j++) {
				for (int i = voxelBounds.getMinX(); i <= voxelBounds.getMaxX(); i++) {
					...
					if (pi.x >= 0 && pi.y >= 0 && pi.x < (int)depthImage.getWidth() && pi.y < (int)depthImage.getHeight()) {
						...
						if (d >= m_depthMin && d <= m_depthMax && std::fabs(d - p.z) <= m_voxelSize) {
							...
							for (unsigned int boxIdx = 0; boxIdx < bboxes.size(); boxIdx++) {
								if (bboxInChunk[boxIdx] < IMAGE_FIND_BBOX_INTERSECT_THRESH) 
									continue;

								const bbox3f& bbox = bboxes[boxIdx].aabb;
								const vec3i bboxmin = math::floor(bbox.getMin());
								const vec3i bboxmax = math::ceil(bbox.getMax());
								if (i >= bboxmin.x && i < bboxmax.x &&
									j >= bboxmin.y && j < bboxmax.y &&
									k >= bboxmin.z && k < bboxmax.z) {
									// check if inside box mask
									if (bboxes[boxIdx].mask(i - bboxmin.x, j - bboxmin.y, k - bboxmin.z) > 0) { //TODO include not in mask ones as well??
										hitSet.insert(vec3i(i, j, k));
										auto it = boxesHit.find(boxIdx);
										if (it == boxesHit.end()) {
											std::unordered_set<vec3i> set; set.insert(vec3i(i, j, k));
											boxesHit[boxIdx] = set;
										}
										else {
											it->second.insert(vec3i(i, j, k));
										}
									}
								}
							}
		...
		return std::make_pair(boxesHit, (unsigned int)hitSet.size());
	}
